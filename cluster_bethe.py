...
mus = np.linspace(0.2625, 0.7375, 20); fmtMu = '%.4f'   # mu / U
Us = np.linspace(1.6, 3.8, 89); fmtU = '%.3f'

# o resto que falta:
#mus = np.linspace(0.25, 0.75, 21); fmtMu = '%.4f'   # mu / U
#Us = np.linspace(1.625, 3.775, 44); fmtU = '%.3f'
...
max_err = 1e-6  # max diff between consecutive Green's functions, 1e-6 recommended on 'acceleration-zitko' article
    ...
    def run(self, copy_file='-'):
        ...
        while diff > self.max_err:
            ...
            if N <= self.tp:
                ...
                Gf, N, status = self.broyden_step(N, self.alpha, freq, Gf_old, dG, dF)
