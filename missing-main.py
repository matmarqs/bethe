...
def main():
    ...
    num = 1; total = len(missing_parameters)
    for missing in missing_parameters:
        U = missing[0]; mu = missing[1]
        ...
        U0 = U; found = False
        copy_file = '-'
        while not found and U0 >= 1.6:
            U0 -= 0.025; U0 = round(U0, 3)
            copy_file = "results/almost/gf-data/gf_bethe-W=2,T=0.004,U=%s,mu=%sU.out" % (fmtU % U0, fmtMu % mu)
            if os.path.exists(copy_file):
                found = True
                print('found')
                break
        U0 = U
        while not found and U0 <= 3.8:
            U0 += 0.025; U0 = round(U0, 3)
            copy_file = "results/almost/gf-data/gf_bethe-W=2,T=0.004,U=%s,mu=%sU.out" % (fmtU % U0, fmtMu % mu)
            if os.path.exists(copy_file):
                found = True
