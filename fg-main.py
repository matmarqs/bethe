...
mu_range = np.linspace(0.4, 0.6, 41)

for mu in mu_range:
    L = []
    i = 0; u = round40(g(mu)); fu = f(mu)
    while u >= fu:
        u = round40(u - 0.025)
        L.append(round(u, 3))
    mydict[round(mu, 4)] = L
...
def main():
    ...
    for mu in mydict:
        start = True
        copy_file = "results/3v3ryth1ng-metal/gf-data/gf_bethe-W=2,T=0.004,U=%s,mu=%sU.out" % (fmtU % mydict[mu][0], fmtMu % mu)
        for U in mydict[mu]:
            ...
            if start:
                ...
                copy_file = "%s/gf_bethe-W=2,T=0.004,U=%s,mu=%sU.out" % (dmft.gf_data, fmtU % (U + 0.025), fmtMu % mu)
