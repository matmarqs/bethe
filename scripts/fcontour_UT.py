...
#fcontour(file, savefig='fig-n.png', title=r'%s path filling $n$ for $\mu = U/2$, $D = 1$' % path,
#        log=False, color='jet', label=r'$n$')

# w = 0, log
fcontour(file, savefig='UT_%s-w0-log.png' % path, title=r'%s path $\rho(\omega=0)$ for $\mu = U/2$, $D = 1$' % path,
        log=True, color='jet', label=r'$\rho(\omega=0)$', trunc=1e-5)

# w = 0, lin
fcontour(file, savefig='UT_%s-w0-lin.png' % path, title=r'%s path $\rho(\omega=0)$ for $\mu = U/2$, $D = 1$' % path,
